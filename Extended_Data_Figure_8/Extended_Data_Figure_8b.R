...
root <- subset(Input1,com=="root")

root$lab <- factor(root$lab,levels = levels(root$lab)[c(3,4,1,2)],ordered = T)


p <- ggplot(root,aes(x=lab, y=y, group=lab, fill=lab))+
...
pdf("myc2protein.pdf", width=4, height=5, useDingbats = F)
p
dev.off()
